...
`agent_setting/pi/`는 Pi coding agent의 지침·설정·extension·agent 등록 정의를 관리합니다. 이 디렉터리를 Pi 로컬 패키지로 등록하고 설정 파일은 심볼릭 링크로 설치합니다. 범용 스킬은 이 저장소에 넣지 않고 별도의 [skills 저장소](https://github.com/alexgim961101/skills)에서 설치합니다.
...
| [../AGENTS.md](../AGENTS.md) | 이 저장소에서 작업할 때 적용되는 프로젝트 전용 지침 | 전역에 링크하지 않음 |
...
| [agents/](agents/) | 보고서 조사자·검증자의 Pi 전용 등록·도구 설정 | `scripts/install.sh`로 `~/.pi/agent/agents/`에 링크 |
...
`~/src/agent_setting/pi`에서 `./scripts/install.sh`로 링크합니다. 기존 `models.json`의 내용이 다르면 설치가 중단되므로 별도의 override를 비교·병합한 뒤 다시 실행하세요. Pi에서 `/model`을 다시 열거나 재시작합니다. `pi install`만으로는 이 설정이 적용되지 않습니다.
...
`pi-lsp-adapter`를 고정 버전으로 설치하고 저장소의 최소 설정을 링크합니다. 기존 `lsp.json`이 다르면 먼저 비교·병합하세요.
...
`~/src/agent_setting/pi`에서 실행합니다. 기존 `~/.config/mcp/mcp.json`이 다르면 덮어쓰지 않으므로 별도 서버/설정을 검토해 저장소 파일에 반영하거나 수동 관리를 유지하세요. 이 파일은 Pi 외 MCP 클라이언트도 사용할 수 있는 공용 경로입니다.
...
`config/web-search.json`은 공식 OpenAI·Codex 엔드포인트의 현재 GPT 모델로 검색을 시도하고, 미지원 모델이나 지정된 오류에서는 Tavily로 폴백합니다. 로컬 파일과 내용이 다르면 설치 스크립트가 이 파일만 보존하고 비정상 종료합니다. 차이를 검토한 뒤 병합하거나 별도 로컬 설정으로 유지하세요. 현재 확장의 `useCurrentModel`은 Claude·Neuralwatt 등 타사 게이트웨이를 지원하지 않으므로 이 경우 Tavily를 사용합니다. Claude 자체 웹 검색 연동은 아직 포함하지 않습니다. 검색과 본문 추출을 분리해 페이지를 읽을 때 검색 API를 다시 호출하지 않습니다.
...
`~/src/agent_setting/pi`에서 실행합니다. 기존 파일이 동일하면 링크로 교체하고, 다른 파일·알 수 없는 링크는 보존하고 오류로 중단합니다. `web-search.json`만 다른 경우에는 나머지를 설치한 뒤 해당 파일을 그대로 두고 비정상 종료합니다. 차이를 수동으로 확인·병합한 뒤 재실행하세요.
...
전역 지침·모델·LSP·Agent Hub·MCP 설정과 보고서 agent 정의가 링크됩니다. Pi를 다시 실행하거나 `/reload`해 반영합니다. Pi는 전역 지침을 프로젝트의 `AGENTS.md`와 함께 읽습니다. 같은 디렉터리의 `AGENTS.override.md`는 우선하며 `--no-context-files`를 쓰면 지침을 읽지 않습니다.

## 지침 수정

`global/AGENTS.md`를 수정하면 링크된 파일에 즉시 반영되지만 실행 중인 Pi에는 `/reload` 또는 재시작이 필요합니다. 저장소를 이동하거나 삭제하면 링크가 끊어지므로 새 경로에서 설치 스크립트를 다시 실행하세요.

API 키가 들어 있는 `~/.pi/agent/auth.json`과 세션 기록, 런타임 `settings.json`은 이 저장소에서 관리하지 않습니다. `settings.json`에는 Pi가 패키지 등록과 테마 선택을 기록하므로 통째로 링크하지 않습니다.

## 스킬 설치 (저장소 밖)

스킬의 원본·설치 스크립트는 [별도 skills 저장소](https://github.com/alexgim961101/skills)에 둡니다. 예: `debug`, `propose`, `plan`, `impl`, `learn`, `frontend-design` 및 운영용 `allinone-rebuild`, `panorama-heatmap-rebuild`, `retailtrend-stack`. 필요할 때만 신뢰할 수 있는 원본과 `SKILL.md`·스크립트를 검토하고 설치합니다.

```bash
git clone git@github.com:alexgim961101/skills.git ~/skills  # 이미 있으면 생략
cd ~/skills
./install.sh --tool pi debug propose plan impl learn frontend-design
```

이 설치 스크립트는 `~/.pi/agent/skills/<스킬명>`에 심볼릭 링크를 만듭니다. 설치할 스킬 목록은 사용 환경에 맞게 바꾸고, Pi에서 `/reload`한 뒤 `/skill:<이름>`으로 확인합니다. 이 저장소에는 스킬 본문을 중복 보관하지 않습니다.
